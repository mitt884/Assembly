...
quit: 
 ...
 findmaxmin: 
	...
	addi $sp, $sp, -48      # load 8 parameters into the stack 
	sw $s0, 44($sp)
	sw $s1, 40($sp)
	sw $s2, 36($sp)
	sw $s3, 32($sp)
...
loop: 	
	...
	sub $t2, $fp, $t2     #t2=address of a[i]
	lw $a0, 0($t2)      #a0 = a[i]
...
min: 
	bge $a0, $a2, next    #if a0>a1 (a[i]=current min), skip to nnext element
	sw $a0, 4($sp)        #else save the new min to the stack
